...
const statusConfig: Record<PlanningStatus, { label: string; color: string; bg: string }> = {
  ...
  proposed:  { label: '已提案', color: 'themeColors.warning', bg: 'themeColors.surface7e6' },
  approved:  { label: '已批准', color: 'themeColors.success', bg: 'themeColors.successBg' },
  rejected:  { label: '已否决', color: 'themeColors.error', bg: 'themeColors.surface2f0' },
};
...
请用中文回复，保持专业简洁，数据要有据可查。`,
  ...
  const marketShareChartOption = {
    ...
    series: [
      { name: '金蝶', type: 'line', data: marketShareTrend.map((d) => d.kingdee), itemStyle: { color: 'chartColors.primary' }, lineStyle: { width: 3 } },
      { name: '用友', type: 'line', data: marketShareTrend.map((d) => d.yonyou), itemStyle: { color: 'chartColors.error' } },
      { name: 'SAP', type: 'line', data: marketShareTrend.map((d) => d.sap), itemStyle: { color: 'chartColors.success' } },
      { name: '浪潮', type: 'line', data: marketShareTrend.map((d) => d.inspur), itemStyle: { color: 'chartColors.warning' } },
    ],
  };
  ...
  const statusPieOption = {
    ...
    series: [{
      ...
      center: ['50%', '50%'],
      data: [
        { value: plans().filter((p) => p.status === 'research').length, name: '调研中', itemStyle: { color: 'themeColors.border' } },
        { value: plans().filter((p) => p.status === 'analyzing').length, name: '分析中', itemStyle: { color: 'chartColors.primary' } },
        { value: plans().filter((p) => p.status === 'proposed').length, name: '已提案', itemStyle: { color: 'chartColors.warning' } },
        { value: plans().filter((p) => p.status === 'approved').length, name: '已批准', itemStyle: { color: 'chartColors.success' } },
      ].filter((d) => d.value > 0),
      label: { formatter: '{b}: {c}' },
    }],
  };
      ...
      <div style={{ display: 'grid', 'grid-template-columns': '2fr 1fr', gap: '16px', 'margin-bottom': '16px' }}>
        ...
        <div style={{ background: 'themeColors.surface', padding: '16px', 'border-radius': '8px', border: '1px solid themeColors.border' }}>
          ...
          <For each={marketInsights.filter((m) => m.impact === 'high')}>
            {(m) => (
              <div style={{ 'margin-bottom': '12px', padding: '8px', background: 'themeColors.surface7e6', 'border-radius': '6px', border: '1px solid themeColors.warningBorder' }}>
