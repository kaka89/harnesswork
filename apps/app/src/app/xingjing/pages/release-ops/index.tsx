...
const ReleaseOps: Component = () => {
    ...
    <div>
      ...
      <div style={{ background: 'themeColors.surface', padding: '16px', 'border-radius': '8px', border: '1px solid themeColors.border', 'margin-bottom': '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '16px', 'align-items': 'flex-end', 'flex-wrap': 'wrap' }}>
          ...
          <button
            style={{
              background: deploying() ? 'themeColors.border' : targetEnv() === 'prod' ? 'chartColors.warning' : 'chartColors.primary',
        ...
        <Show when={deploying() || deployDone()}>
          <div style={{ 'margin-top': '14px' }}>
            <div style={{ background: 'themeColors.primaryBg', height: '8px', 'border-radius': '4px', overflow: 'hidden' }}>
              <div style={{
                background: deploying() ? 'chartColors.primary' : 'chartColors.success',
                    ...
                    <tr style={{ 'border-bottom': '1px solid themeColors.border', cursor: 'pointer' }}>
                      ...
                      <td style={{ padding: '8px' }}>
                        <span style={{
                          ...
                          color: run.targetEnv === 'dev' ? 'chartColors.primary' : run.targetEnv === 'staging' ? 'chartColors.purple' : 'themeColors.success',
                            ...
                            {(stage) => (
                              <span
                                ...
                                style={{
                                  ...
                                  background: stage.status === 'success' ? 'chartColors.success' : stage.status === 'failed' ? 'chartColors.error' : stage.status === 'running' ? 'chartColors.primary' : 'themeColors.border',
                                }}
                              />
                            )}
                          </For>
                      ...
                      <td style={{ padding: '8px' }}>
                        <div style={{ display: 'flex', gap: '6px' }}>
                          ...
                          <Show when={run.status === 'success' && run.targetEnv === 'prod'}>
                            <button style={{ background: 'transparent', border: '1px solid chartColors.error', color: 'chartColors.error', padding: '2px 8px', 'border-radius': '3px', cursor: 'pointer', 'font-size': '11px', display: 'flex', 'align-items': 'center', gap: '3px' }}>
                      ...
                      <tr>
                        <td colSpan={9} style={{ padding: '12px 16px', background: 'themeColors.hover', 'border-bottom': '1px solid themeColors.border' }}>
                          ...
                          <div style={{ display: 'flex', 'flex-wrap': 'wrap', gap: '8px' }}>
                            <For each={run.stages}>
                              {(stage) => (
                                <div style={{ background: 'themeColors.surface', padding: '6px 12px', 'border-radius': '4px', 'border-left': `3px solid ${stage.status === 'success' ? 'chartColors.success' : stage.status === 'failed' ? 'chartColors.error' : stage.status === 'running' ? 'chartColors.primary' : 'themeColors.border'}` }}>
  ...
  const renderMonitoring = () => (
    <div>
      ...
      <div style={{ display: 'grid', 'grid-template-columns': 'repeat(4, 1fr)', gap: '16px', 'margin-bottom': '16px' }}>
        {[
          { title: '系统可用性', value: '99.82%', color: 'chartColors.success' },
          { title: '平均响应时间', value: `${avg('responseTime')}ms`, color: avg('responseTime') > 200 ? 'chartColors.warning' : 'chartColors.primary' },
          { title: '平均错误率', value: `${(systemMetrics.reduce((s, p) => s + p.errorRate, 0) / systemMetrics.length).toFixed(2)}%`, color: 'chartColors.warning' },
          { title: '平均 CPU', value: `${avg('cpu')}%`, color: 'chartColors.purple' },
        ].map((m) => (
          <div style={{ background: 'themeColors.surface', padding: '16px', 'border-radius': '8px', border: '1px solid themeColors.border' }}>
              ...
              <For each={servicesHealth}>
                {(svc) => (
                  <tr style={{ 'border-bottom': '1px solid themeColors.border', background: svc.status === 'degraded' ? 'themeColors.warningBg' : 'transparent' }}>
                    ...
                    <td style={{ padding: '8px', color: svc.responseTime > 300 ? 'chartColors.error' : svc.responseTime > 200 ? 'chartColors.warning' : 'chartColors.success', 'font-weight': '600' }}>{svc.responseTime} ms</td>
                    <td style={{ padding: '8px', color: svc.errorRate > 1 ? 'chartColors.error' : svc.errorRate > 0.5 ? 'chartColors.warning' : 'chartColors.success' }}>{svc.errorRate}%</td>
                    <td style={{ padding: '8px' }}>{svc.instances} 个</td>
                    <td style={{ padding: '8px' }}><span style={{ background: 'themeColors.primaryBg', color: 'chartColors.primary', padding: '1px 7px', 'border-radius': '3px' }}>{svc.version}</span></td>
                  </tr>
                )}
              </For>
            </tbody>
      ...
      <div style={{ background: 'themeColors.surface', padding: '16px', 'border-radius': '8px', border: '1px solid themeColors.border', 'margin-bottom': '16px' }}>
        <div style={{ display: 'flex', 'justify-content': 'space-between', 'align-items': 'center', 'margin-bottom': '12px' }}>
          <div style={{ 'font-weight': '600', display: 'flex', 'align-items': 'center', gap: '6px' }}>
            <AlertTriangle size={14} style={{ color: 'chartColors.error' }} />告警事件
          </div>
              ...
              <div style={{ display: 'flex', 'justify-content': 'space-between', 'align-items': 'flex-start' }}>
                ...
                <div style={{ display: 'flex', 'align-items': 'center', gap: '8px', 'flex-shrink': '0', 'margin-left': '12px' }}>
                  ...
                  <Show when={alert.status === 'firing'}>
                    <button
                      style={{ background: 'themeColors.primaryBg', border: '1px solid themeColors.primaryBorder', color: 'chartColors.primary', padding: '3px 9px', 'border-radius': '3px', cursor: 'pointer', 'font-size': '11px' }}
              ...
              <For each={errorTraces}>
                {(err) => (
                  <tr style={{ 'border-bottom': '1px solid themeColors.border' }}>
                    ...
                    <td style={{ padding: '8px', 'font-weight': 'bold', color: err.count > 100 ? 'chartColors.error' : 'chartColors.warning' }}>{err.count}</td>
                    <td style={{ padding: '8px' }}>
                      {err.affectedUsers > 0
  ...
  const renderIntegrations = () => (
    <div>
      <div style={{ background: 'themeColors.primaryBg', border: '1px solid themeColors.primaryBorder', padding: '10px 14px', 'border-radius': '6px', 'margin-bottom': '16px', 'font-size': '12px', color: 'chartColors.primary' }}>
        ...
        <For each={integrationList()}>
          {(item) => (
            <div style={{
              ...
              'border-left': `3px solid ${item.connected ? 'chartColors.success' : 'themeColors.border'}`,
            }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                ...
                <Show when={item.connected && item.endpoint}>
                  <button style={{ background: 'transparent', border: '1px solid themeColors.primaryBorder', color: 'chartColors.primary', padding: '3px 10px', 'border-radius': '3px', cursor: 'pointer', 'font-size': '11px', display: 'flex', 'align-items': 'center', gap: '3px' }}
                ...
                <button
                  style={{
                    ...
                    border: `1px solid ${item.connected ? 'chartColors.error' : 'chartColors.success'}`,
                    color: item.connected ? 'chartColors.error' : 'chartColors.success',
    ...
    <div>
      <div style={{ 'margin-bottom': '16px', display: 'flex', 'justify-content': 'space-between', 'align-items': 'center' }}>
        <h2 style={{ margin: 0, 'font-size': '20px', display: 'flex', 'align-items': 'center', gap: '8px' }}>
          <Server size={20} style={{ color: 'chartColors.primary' }} />发布与运维中心
        </h2>
        <div style={{ display: 'flex', gap: '8px', 'align-items': 'center' }}>
          ...
          <span style={{ background: 'themeColors.primaryBg', color: 'chartColors.primary', border: '1px solid themeColors.primaryBorder', padding: '3px 10px', 'border-radius': '4px', 'font-size': '12px' }}>
            上线 3 天 | 可用性 99.82%
          </span>
        </div>
      </div>
          ...
          {(tab) => (
            ...
            >
              ...
              <Show when={(tab as any).badge > 0}>
                <span style={{ background: 'chartColors.error', color: 'themeColors.surface', 'font-size': '10px', padding: '1px 5px', 'border-radius': '8px' }}>
                  {(tab as any).badge}
                </span>
              </Show>
            </button>
