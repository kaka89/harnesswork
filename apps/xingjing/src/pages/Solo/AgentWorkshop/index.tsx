...
import {
  TeamOutlined, PlusOutlined, CheckCircleOutlined, DeleteOutlined,
  ClockCircleOutlined, PlayCircleOutlined, ThunderboltOutlined,
  RobotOutlined, EditOutlined, ExclamationCircleOutlined, LoadingOutlined,
...
import {
  soloSkillPool, SkillDef, SkillInputParam, AgentAssignment, initialSoloAssignments,
  agentColorPresets, emojiPresets, ColorPreset,
  TaskOrchestration, soloOrchestrations,
} from '../../../mock/agentWorkshop';
import { soloTasks } from '../../../mock/solo';

const { Text, Title, Paragraph } = Typography;
const { TextArea } = Input;

const soloTaskStatusTag: Record<string, { label: string; color: string }> = {
...
const assignStatusIcon: Record<string, React.ReactNode> = {
  ...
  working:  <PlayCircleOutlined style={{ color: '#52c41a' }} />,
...
const skillStatusConfig: Record<string, { color: string; text: string; icon: React.ReactNode }> = {
  ...
  running: { color: '#52c41a', text: '执行中', icon: <LoadingOutlined /> },
  pending: { color: '#d9d9d9', text: '待执行', icon: <ClockCircleOutlined /> },
};

const categoryColor: Record<string, string> = {
  '产品': '#1264e5', '工程': '#08979c', '增长': '#d46b08', '运营': '#389e0d',
};

const soloCategoryOptions = ['产品', '工程', '增长', '运营'];

let soloAgentIdCounter = 200;
let soloSkillIdCounter = 600;
          ...
          <Space size={4} style={{ flexShrink: 0 }}>
            {skillDef && (
              <Tooltip title="编辑 Skill">
                <EditOutlined style={{ fontSize: 12, color: '#52c41a', cursor: 'pointer' }} onClick={() => onEdit(skillDef)} />
              </Tooltip>
            )}
            <Tooltip title="移除">
              <DeleteOutlined style={{ fontSize: 12, color: '#bfbfbf', cursor: 'pointer' }} onClick={onRemove} />
            </Tooltip>
          </Space>
        </div>
      </Card>
    ...
    <div ref={setNodeRef} style={style}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <div {...listeners} {...attributes} style={{ cursor: 'grab', color: '#b7eb8f', flexShrink: 0 }}>
          ...
          <Space size={4} style={{ flexShrink: 0 }}>
            <Tooltip title="编辑">
              <EditOutlined style={{ fontSize: 11, color: '#52c41a', cursor: 'pointer' }} onClick={() => onEdit(skill)} />
            </Tooltip>
...
const AgentSkillDropZone: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { isOver, setNodeRef } = useDroppable({ id: 'solo-agent-skill-list' });
  return (
    <div
      ref={setNodeRef}
      style={{
        minHeight: 60,
        border: `2px dashed ${isOver ? '#52c41a' : 'transparent'}`,
        borderRadius: 8,
        background: isOver ? '#f6ffed' : 'transparent',
        transition: 'all 0.2s',
        padding: isOver ? 4 : 0,
      }}
    >
  ...
  const handleOk = () => {
    form.validateFields().then((values) => {
      const skill: SkillDef = {
        id: editingSkill?.id || `ss-custom-${++soloSkillIdCounter}`,
        name: values.name,
    ...
    >
      <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
        ...
        <Form.Item name="trigger" label="触发方式">
          <Input placeholder="如：用户手动调用 / 事件触发" />
        </Form.Item>
        ...
        <Form.Item name="outputType" label="输出类型">
          <Input placeholder="如：file / list[Feature] / string" />
        </Form.Item>
        ...
        <Form.Item label="输入参数">
          <div style={{ border: '1px solid #d9f7be', borderRadius: 8, padding: 12, background: '#f6ffed' }}>
              ...
              >
                ...
                <Tooltip title="必填">
                  <Switch
                    ...
                    style={param.required ? { background: '#52c41a' } : {}}
                  />
                </Tooltip>
                <Input
            ...
            <Button size="small" icon={<PlusOutlined />} type="dashed" onClick={addParam} style={{ width: '100%', borderColor: '#b7eb8f', color: '#52c41a' }}>
              添加参数
            </Button>
          </div>
        </Form.Item>
...
// ─── Main Component ─────────────────────────────────────────────────────
const SoloAgentWorkshop: React.FC = () => {
  const [agents, setAgents] = useState<AgentDef[]>([...soloAgents]);
  const [agentSkills, setAgentSkills] = useState<Record<string, string[]>>(() => {
    const map: Record<string, string[]> = {};
    soloAgents.forEach((a) => { map[a.id] = [...a.skills]; });
    return map;
  });
  const [assignments, setAssignments] = useState<AgentAssignment[]>([...initialSoloAssignments]);
  const [orchestrations, setOrchestrations] = useState<TaskOrchestration[]>([...soloOrchestrations]);
  const [selectedAgent, setSelectedAgent] = useState<AgentDef | null>(null);
  const [drawerOpen, setDrawerOpen] = useState(false);
  const [pendingTasks, setPendingTasks] = useState<string[]>([]);
  const [skillPool, setSkillPool] = useState<SkillDef[]>([...soloSkillPool]);

  const [modalOpen, setModalOpen] = useState(false);
  const [editingAgent, setEditingAgent] = useState<AgentDef | null>(null);
  const [agentForm] = Form.useForm();
  const [selectedEmoji, setSelectedEmoji] = useState('🧠');
  const [selectedColor, setSelectedColor] = useState<ColorPreset>(agentColorPresets[4]);
  ...
  const addSkill = (agentId: string, skillName: string) => {
    if ((agentSkills[agentId] || []).includes(skillName)) {
      message.warning(`${skillName} 已在该 Agent 中`);
      return;
    }
  ...
  const findSkillDef = (name: string): SkillDef | undefined => skillPool.find((s) => s.name === name);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (activeId.startsWith('pool__')) {
      ...
      if (overId === 'solo-agent-skill-list' || !overId.startsWith('pool__')) {
  ...
  // ─── Skill Modal ───
  const openNewSkillModal = () => { setEditingSkill(null); setSkillModalOpen(true); };
  const openEditSkillModal = (skill: SkillDef) => { setEditingSkill(skill); setSkillModalOpen(true); };
  ...
  const handleSkillSave = (skill: SkillDef) => {
    if (editingSkill) {
      ...
      if (selectedAgent) addSkill(selectedAgent.id, skill.name);
  ...
  const openEditModal = (agent: AgentDef) => {
    ...
    const matchColor = agentColorPresets.find((c) => c.color === agent.color) || agentColorPresets[4];
  ...
  const handleModalOk = () => {
    agentForm.validateFields().then((values) => {
      if (editingAgent) {
        ...
        const newId = `custom-solo-${++soloAgentIdCounter}`;
  ...
  const handleDelete = (agent: AgentDef) => {
    Modal.confirm({
      ...
      content: '删除后该 Agent 的所有配置和任务指派将被清除。',
  ...
  return (
    <div>
      <Alert
        type="success" showIcon icon={<TeamOutlined />}
        message="独立版 AI搭档"
        description="你的虚拟 AI 搭档团队。支持拖拽排序 Skill、从 Skill 池拖入 AI搭档、新建和编辑 Skill 规格。每个 AI搭档通过调度 Skill 完成指派的任务。"
        ...
        {agents.map((agent) => {
          ...
          return (
            <Col xs={24} sm={12} lg={6} key={agent.id}>
              <Badge count={count} offset={[-8, 8]} color={agent.color}>
                <Card
                  hoverable onClick={() => openDrawer(agent)}
                  style={{ borderRadius: 12, border: `1px solid ${agent.borderColor}`, background: `linear-gradient(135deg, ${agent.bgColor} 0%, #fff 100%)`, width: '100%' }}
                >
              ...
              items={[
                {
                  ...
                  children: (
                    ...
                    >
                      <div style={{ marginBottom: 12 }}>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                          <Text strong style={{ fontSize: 13 }}>
                            ...
                            搭档的 Skill
                            <Tag style={{ marginLeft: 6 }} color="green">{(agentSkills[selectedAgent.id] || []).length}</Tag>
                          </Text>
                          <Button size="small" type="dashed" icon={<PlusOutlined />} onClick={openNewSkillModal}
                            style={{ borderColor: '#b7eb8f', color: '#52c41a' }}>
                            新建 Skill
                          </Button>
                        </div>
                      ...
                      <DragOverlay>
                        {activeSkillDragId ? (
                          <div style={{ padding: '6px 12px', background: '#52c41a', color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 500, boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
                ...
                },
                {
                  ...
                  children: (
                    <div>
                            ...
                            {soloTasks.map((task) => {
                              ...
                              return (
                                <Checkbox key={task.id} value={task.id} style={{ width: '100%' }} disabled={!!otherAgent}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    <Tag color={typeColor[task.type] || 'default'} style={{ fontSize: 11 }}>{task.type}</Tag>
                                    <Text style={{ fontSize: 13, flex: 1 }}>{task.title}</Text>
                                    <Tag color={soloTaskStatusTag[task.status]?.color} style={{ fontSize: 11 }}>{soloTaskStatusTag[task.status]?.label}</Tag>
                      ...
                      <Button type="primary" icon={<CheckCircleOutlined />} onClick={confirmAssignment}
                        style={{ width: '100%', marginBottom: 24, background: '#52c41a', borderColor: '#52c41a' }}>
                      ...
                      {assignments.filter((a) => a.agentId === selectedAgent.id).length === 0 ? (
                        ...
                        <Collapse
                          ...
                          items={assignments
                            ...
                            .map((a) => {
                              const task = soloTasks.find((t) => t.id === a.taskId);
                              ...
                              return {
                                ...
                                label: (
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                    ...
                                    {task && (
                                      <>
                                        <Tag color={typeColor[task.type] || 'default'} style={{ fontSize: 10 }}>{task.type}</Tag>
                                        <Tag color={soloTaskStatusTag[task.status]?.color} style={{ fontSize: 10 }}>{soloTaskStatusTag[task.status]?.label}</Tag>
                                      </>
                                    )}
                                  </div>
                                ),
                                children: orch ? (
                                  ...
                                  <div style={{ padding: '12px 0', textAlign: 'center' }}>
                                    <Text type="secondary" style={{ fontSize: 12 }}><ClockCircleOutlined style={{ marginRight: 6 }} />等待 Agent 调度...</Text>
                                  </div>
                                ),
                              };
                            })}
      ...
      <Modal
        title={editingAgent ? `编辑: ${editingAgent.name}` : '创建新的 AI搭档'}
        open={modalOpen} onOk={handleModalOk}
        onCancel={() => { setModalOpen(false); agentForm.resetFields(); }}
        okText={editingAgent ? '保存' : '创建'} cancelText="取消" width={520}
        okButtonProps={{ style: { background: '#52c41a', borderColor: '#52c41a' } }}
      >
        <Form form={agentForm} layout="vertical" style={{ marginTop: 16 }}>
          <Form.Item name="name" label="名称" rules={[{ required: true, message: '请输入名称' }]}>
            <Input placeholder="如：数据搭档" />
          </Form.Item>
          ...
          <Form.Item name="description" label="职责描述" rules={[{ required: true, message: '请输入描述' }]}>
            <TextArea rows={2} placeholder="一句话描述核心职责" />
          </Form.Item>
              ...
              {emojiPresets.map((e) => (
                <div key={e} onClick={() => setSelectedEmoji(e)}
                  style={{ width: 40, height: 40, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, borderRadius: 8, cursor: 'pointer',
                    border: selectedEmoji === e ? '2px solid #52c41a' : '1px solid #f0f0f0', background: selectedEmoji === e ? '#f6ffed' : '#fafafa' }}>
                  {e}
                </div>
              ))}
            </div>
          ...
          <Form.Item label="预览">
            <Card size="small" style={{ borderRadius: 10, border: `1px solid ${selectedColor.borderColor}`, background: `linear-gradient(135deg, ${selectedColor.bgColor} 0%, #fff 100%)` }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div>
                  <Text strong style={{ fontSize: 13 }}>{agentForm.getFieldValue('name') || '名称'}</Text>
                  <br />
      ...
      <SkillEditModal
        ...
        categoryOptions={soloCategoryOptions}
      />
    </div>
  );
};
