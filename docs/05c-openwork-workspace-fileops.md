...
OpenWork 平台对外提供「workspace」概念作为一切文件操作、会话、扩展的根容器。本文档以 `apps/server/src/` 下的服务端文件和 `apps/app/src/` 下的客户端代码为信息源，回答以下问题：
...
7. 客户端 React 层如何管理 workspace 状态（Zustand store）
...
客户端的 `WorkspaceInfo` 类型定义于 [apps/app/src/app/lib/desktop-tauri.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/desktop-tauri.ts#L127-L148)，包含 `workspaceType`、`remoteType`、`openworkHostUrl`、`openworkWorkspaceId` 等字段。`OpenworkWorkspaceInfo`（定义于 [openwork-server.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/openwork-server.ts#L94-L101)）在 `WorkspaceInfo` 基础上扩展了远端 opencode 连接字段。
...
客户端的 `WorkspacePreset` 类型（`"starter" | "automation" | "minimal"`）定义于 [apps/app/src/app/types.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/types.ts#L183)。
...
### 4.4 客户端调用（`OpenworkServerClient`）

客户端通过 [createOpenworkServerClient](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/openwork-server.ts#L708) 工厂创建的客户端对象（类型 `OpenworkServerClient`）封装了此接口：

```ts
// 读文件
client.readWorkspaceFile(workspaceId, path)
  // → Promise<OpenworkWorkspaceFileContent>
  // 返回 { path, content, bytes, updatedAt }

// 写文件
client.writeWorkspaceFile(workspaceId, { path, content, baseUpdatedAt?, force? })
  // → Promise<OpenworkWorkspaceFileWriteResult>
  // 返回 { ok, path, bytes, updatedAt, revision? }
```
...
### 7.3 客户端 API（`OpenworkServerClient`）

```ts
// 导出 workspace
client.exportWorkspace(workspaceId, { sensitiveMode: "auto" })
  // → Promise<OpenworkWorkspaceExport>
  // 返回 { workspaceId, exportedAt, opencode, openwork, skills, commands, files }

// 导入 workspace
client.importWorkspace(workspaceId, payload)
  // → Promise<{ ok: boolean }>
```

`OpenworkWorkspaceExport` 和 `OpenworkWorkspaceExportSensitiveMode` 定义于 [openwork-server.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/openwork-server.ts#L194-L210)。

Desktop 端还提供原生命令封装，通过 [workspaceExportConfig](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/desktop-tauri.ts#L312-L319) 和 [workspaceImportConfig](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/desktop-tauri.ts#L322-L332) 直接调用 Electron/Tauri IPC。
...
## 11. 客户端 React 层：Workspace 状态管理

### 11.1 Zustand Store（`useOpenworkStore`）

全局 workspace 状态由 [apps/app/src/react-app/kernel/store.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/react-app/kernel/store.ts) 中的 Zustand store 管理：

```ts
export type OpenworkStore = {
  bootstrapping: boolean;
  server: ServerState;
  workspaces: OpenworkWorkspaceInfo[];   // 全部 workspace 列表
  activeWorkspaceId: string | null;      // 当前激活的 workspace
...
常用 selectors 定义于 [kernel/selectors.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/react-app/kernel/selectors.ts)：

```ts
// 获取当前激活的 workspace
const activeWorkspace = useOpenworkStore(selectActiveWorkspace);
// 等价于：workspaces.find(w => w.id === activeWorkspaceId) ?? null
```

这取代了旧版 SolidJS 中基于 Signal/Context 的响应式方案。

### 11.2 Desktop 层 Workspace 操作（IPC 命令）

桌面应用通过 `desktopBridge`（[apps/app/src/app/lib/desktop.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/desktop.ts)）自动路由到 Electron bridge 或 Tauri 原生命令。workspace 管理函数定义于 [desktop-tauri.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/desktop-tauri.ts)：

| 函数 | IPC 命令 | 返回值 |
|---|---|---|
| `workspaceBootstrap()` | `workspace_bootstrap` | `WorkspaceList` |
| `workspaceCreate({ folderPath, name, preset })` | `workspace_create` | `WorkspaceList` |
| `workspaceCreateRemote(input)` | `workspace_create_remote` | `WorkspaceList` |
| `workspaceUpdateRemote(input)` | `workspace_update_remote` | `WorkspaceList` |
| `workspaceSetSelected(workspaceId)` | `workspace_set_selected` | `WorkspaceList` |
| `workspaceSetRuntimeActive(workspaceId)` | `workspace_set_runtime_active` | `WorkspaceList` |
| `workspaceUpdateDisplayName(input)` | `workspace_update_display_name` | `WorkspaceList` |
| `workspaceForget(workspaceId)` | `workspace_forget` | `WorkspaceList` |
| `workspaceExportConfig({ workspaceId, outputPath })` | `workspace_export_config` | `WorkspaceExportSummary` |
| `workspaceImportConfig({ archivePath, targetDir, name })` | `workspace_import_config` | `WorkspaceList` |
| `workspaceOpenworkRead({ workspacePath })` | `workspace_openwork_read` | `WorkspaceOpenworkConfig` |
| `workspaceOpenworkWrite({ workspacePath, config })` | `workspace_openwork_write` | `ExecResult` |

所有操作均返回最新的 `WorkspaceList`（含 `selectedId`、`watchedId`、`workspaces[]`），调用方负责将结果同步到 Zustand store。

```ts
// 典型调用模式（来自 settings-route.tsx）
const list = await workspaceCreate({ folderPath, name, preset });
const createdId = resolveWorkspaceListSelectedId(list);
await workspaceSetSelected(createdId);
```

### 11.3 `resolveWorkspaceListSelectedId` 辅助函数
...
> **向后兼容**：旧版桌面载荷使用 `activeId` 表示 UI 选中的 workspace，`selectedId` 是新字段；此函数同时处理两者。

### 11.4 Server 模式下的 Workspace 操作（`OpenworkServerClient`）

当以 server 模式连接远程 OpenWork 时，使用 [createOpenworkServerClient](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/openwork-server.ts#L708) 创建客户端：

```ts
const client = createOpenworkServerClient({
  baseUrl: "https://your-openwork.example.com",
  token: "...",
  hostToken: "...",
});
...
`buildOpenworkWorkspaceBaseUrl(hostUrl, workspaceId)` 将 host URL 与 workspace ID 组合为 workspace-scoped 的 opencode 代理 URL（形如 `<host>/w/<workspaceId>`），该 URL 可直接用于 opencode SDK 客户端初始化。

### 11.5 React 数据流层次

```mermaid
graph TD
  A[desktopBridge / IPC] -->|WorkspaceList| B[shell/settings-route.tsx]
  C[OpenworkServerClient] -->|OpenworkWorkspaceList| D[shell/session-route.tsx]
  B --> E[useState: workspaces]
  D --> E
  E -->|同步| F[useOpenworkStore Zustand]
  F -->|selectActiveWorkspace| G[domain components]
  F -->|server.status / server.capabilities| G
```
...
sequenceDiagram
  ...
  SV-->>FE: { items, cursor }
  FE->>ZS: setActiveWorkspaceId / 触发 re-render
```
...
| 23 | Zustand store 管理全局 workspace 状态 | [kernel/store.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/react-app/kernel/store.ts) | 取代 SolidJS Signal/Context；React 19 组件直接 hook 订阅 |
| 24 | IPC 命令与 HTTP API 两条调用路径 | [desktop-tauri.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/desktop-tauri.ts) / [openwork-server.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/openwork-server.ts) | 本地桌面用 IPC（Electron/Tauri）；server 模式用 REST |
| 25 | `desktopBridge` proxy 自动路由 Electron/Tauri | [desktop.ts](file:///Users/umasuo_m3pro/Desktop/startup/xingjing/harnesswork/apps/app/src/app/lib/desktop.ts) | 调用方无需感知运行时；统一接口 |
...
| [05h-openwork-state-architecture.md](./05h-openwork-state-architecture.md) | 「文件系统是单一事实源」与 React Provider 层次（ServerProvider / GlobalSDKProvider / GlobalSyncProvider / LocalProvider）的对应 |
